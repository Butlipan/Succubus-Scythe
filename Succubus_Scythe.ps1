...
function applications {
    try {
        Write-Host "`n[+] Checking installed applications..." -ForegroundColor Cyan
        $INSTALLED = Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Select-Object DisplayName, DisplayVersion, InstallLocation
        $INSTALLED += Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Select-Object DisplayName, DisplayVersion, InstallLocation
        $INSTALLED | Where-Object { $_.DisplayName } | Sort-Object DisplayName -Unique | Select-Object DisplayName, @{Name="Version";Expression={ if ($_.DisplayVersion) { $_.DisplayVersion } else { "N/A" } }}, InstallLocation | Format-Table -AutoSize
    }
    catch {
        Write-Host "[-] NO PERMISSIONS OR ERROR" -ForegroundColor Red
    }
...
stripe
wait
Write-Host '=========  INSTALLED APPLICATIONS  ==========' -ForegroundColor Magenta
applications | Out-Host
stripe
wait
